...
TEST_DIR ="/home/vasu/Documents/palmoilclassification/input/Test/"
BATCH_SIZE = 1
DEVICE = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
IMAGE_RESIZE = 224
NUM_WORKERS = 4
CLASS_NAMES = ['Labile', 'Metastable', 'Stable']
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(testloader), total=len(testloader)):
            ...
            save_name = os.path.join(
                '/home/vasu/Documents/palmoilclassification/',
                'outputs',
                'cam_results',
                str(counter)+'.png'
            )
...
if __name__ == '__main__':
    os.makedirs(os.path.join('/home/vasu/Documents/palmoilclassification/outputs/', 'cam_results'), exist_ok=True)

    dataset_test = get_datasets(IMAGE_RESIZE)
    test_loader = get_data_loader(dataset_test)

    checkpoint = torch.load(os.path.join('/home/vasu/Documents/palmoilclassification/outputs/', 'model.pth'))
    # Load the model.
    model = build_model(
        pretrained=False,
        fine_tune=False, 
